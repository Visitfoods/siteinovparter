...
export const toggleUserActive = async (userId: string): Promise<void> => {
  try {
    ...
    // Erro ao alternar estado do utilizador
    throw error;
  }
};
  ...
  try {
    ...
    if (existingUsers.empty) {
      const initialUsers = [
        {
          username: 'admin',
          password: await bcrypt.hash(process.env.DEFAULT_GUIDE_PASSWORD || 'guiareal123', 10),
          role: 'user' as const,
          active: true,
        ...
        {
          username: 'superadmin',
          password: await bcrypt.hash(process.env.DEFAULT_ADMIN_PASSWORD || 'superadmin123', 10),
          role: 'admin' as const,
          active: true,
    ...
    // Erro ao criar utilizadores iniciais
    throw error;
  }
};