...
export default function ConversationsPage() {
  ...
  // Definindo o guia fixo como "inovpartner"
  const [selectedGuide, setSelectedGuide] = useState('inovpartner');
  // Não precisamos mais de uma lista de guias disponíveis
  const [stats, setStats] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  // Removida a variável de carregamento de guias
  ...
  useEffect(() => {
    ...
    debugLog('Project ID:', 'inovpartner-23bc8');
    // Limpar conversas ao mudar de guia
    setConversations([]);
    setSelectedConversation(null);

    const unsubscribe = listenToGuideConversations('inovpartner-23bc8', selectedGuide, (convs) => {
  ...
  // Função simplificada para usar apenas o guia "inovpartner"
  const loadAvailableGuides = async () => {
    // Definindo diretamente o guia "inovpartner"
    setSelectedGuide('inovpartner');
  };
  ...
  const loadConversations = async () => {
    ...
    try {
      ...
      debugLog('Project ID:', 'inovpartner-23bc8');
      const data = await listGuideConversations('inovpartner-23bc8', selectedGuide);
  ...
  const loadStats = async () => {
    ...
    try {
      const data = await getGuideStats('inovpartner-23bc8', selectedGuide);
      setStats(data);
    } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await sendGuideMessage('inovpartner-23bc8', selectedConversation.id!, {
        from: 'guide',
        text: newMessage,
        metadata: {
          guideResponse: true
  ...
  const handleCloseConversation = async (conversationId: string) => {
    try {
      await closeGuideConversation('inovpartner-23bc8', conversationId, 'admin', 'Fechada pelo administrador');
  ...
  const handleReopenConversation = async (conversationId: string) => {
    try {
      await reopenGuideConversation('inovpartner-23bc8', conversationId);
      await loadConversations();
    } catch (error) {
  ...
  const handleDeleteConversation = async (conversationId: string) => {
    try {
      ...
      if (!ok) return;
      await deleteGuideConversation('inovpartner-23bc8', conversationId);
      await loadConversations();
      if (selectedConversation?.id === conversationId) {
        setSelectedConversation(null);
      }
  ...
  const handleMarkAsRead = async (conversationId: string) => {
    try {
      await markGuideMessagesAsRead('inovpartner-23bc8', conversationId);
      await loadConversations();
    } catch (error) {
  ...
  const handleAddTags = async (conversationId: string) => {
    ...
    try {
      ...
      await addGuideConversationTags('inovpartner-23bc8', conversationId, tags);
      await loadConversations();
      setNewTags('');
    } catch (error) {
  ...
  const createTestConversation = async () => {
    try {
      ...
      const conversationId = await createGuideConversation('inovpartner-23bc8', testConversation);
            ...
            <div className={styles.toolbarLeft}>
              ...
              {role === 'admin' && (
                <div className={styles.dropdownGroup}>
                  ...
                  <div className={[styles.dropdownSelect, styles.guideHighlight, styles.guideSelectWide].join(' ')} style={{ width: 597, padding: '8px 12px' }}>
                    Inov Partner
                  </div>
                </div>
              )}
