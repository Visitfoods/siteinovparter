...
export default function Home({ guideVideos, guideSlug, onShowActionButtonsChange }: HomeProps) {
  // Hooks de otimização
  ...
  const getVideoClassName = (baseClass: string) => {
    const { devicePerformance } = videoOptimization;
    if (devicePerformance === 'low') {
      return `${baseClass} optimized-low`;
    } else if (devicePerformance === 'very-low') {
      return `${baseClass} optimized-very-low`;
    }
    return baseClass;
  };

  // Otimização: pausar vídeos em background em dispositivos fracos
  useEffect(() => {
    const { devicePerformance } = videoOptimization;
    ...
    if (devicePerformance === 'very-low' || devicePerformance === 'low') {
      const handleVisibilityChange = () => {
        if (document.hidden) {
          // Pausar todos os vídeos quando a página não está visível
  ...
  // Função simplificada para URLs de vídeo - sem sistema antigo de proxy
  const toStreamUrl = (url?: string | null) => {
    if (!url) return '';
    // Para vídeos estáticos locais, retornar o URL diretamente
    if (url.startsWith('/')) {
      return url;
    }
    // Para URLs externos, retornar como estão
    return url;
  };
  ...
  useEffect(() => {
    try {
      ...
      // Removido: sistema antigo de vídeos externos
  ...
  useEffect(() => {
    ...
    if (isRefresh || isIOS) {
      ...
      if (conversationId) {
        closeGuideConversation('inovpartner-23bc8', conversationId, 'user', 'Fechada no refresh').catch(error => {
          // Erro ao encerrar conversa no refresh
        });
  ...
  useEffect(() => {
    ...
    if (isIOS) {
      ...
      if (conversationId) {
        ...
        closeGuideConversation('inovpartner-23bc8', conversationId, 'user', 'Fechada no carregamento iOS').catch(error => {
          // Erro ao encerrar conversa no carregamento iOS
        });
    ...
    (window as { openGuiaReal?: () => void }).openGuiaReal = () => {
      ...
      if (conversationId && userName && userContact) {
        ...
        if (!currentConversation) {
          unsubscribeRef.current = listenToGuideConversation('inovpartner-23bc8', conversationId, (conv) => {
  ...
  useEffect(() => {
    ...
    if (showChatbotPopup || showHumanChat) {
      ...
      // Forçar position: fixed no container do vídeo
      if (videoContainer) {
        ...
        videoContainer.style.setProperty('min-height', '100svh', 'important');
      ...
      document.body.style.overflow = 'auto';
      // Restaurar posição original do container do vídeo
      if (videoContainer) {
        if (showActionButtons) {
          videoContainer.style.setProperty('position', 'sticky');
        } else {
          videoContainer.style.setProperty('position', 'fixed');
        }
        videoContainer.style.removeProperty('overflow');
      }
      // No Android, forçar um reflow para garantir que o scroll seja restaurado
      if (isAndroid) {
        void document.body.offsetHeight; // Trigger reflow
    ...
    if (conversationId && userName && userContact) {
      setHasActiveSession(true);
        ...
        if (showHumanChat && !isCreatingNewConversation) {
          unsubscribeRef.current = listenToGuideConversation('inovpartner-23bc8', conversationId, (conversation) => {
          // Verificar se a conversa foi fechada no backoffice
            ...
            if (!hasClosingMessage) {
              ...
              sendGuideMessage('inovpartner-23bc8', conversationId, {
                from: 'guide',
  ...
  useEffect(() => {
    ...
    const checkConversationStatus = async () => {
      try {
        // Usar getGuideConversation para verificar o estado da conversa no Firebase
        const conversation = await getGuideConversation('inovpartner-23bc8', conversationId);
        ...
        // Erro ao verificar estado da conversa
    ...
    if (!isDesktop && showGuidePopup) {
      ...
      if (videoRef.current) {
        try {
          ...
          // Erro ao pausar vídeo principal
        }
      }
  ...
  useEffect(() => {
    ...
    if (!showGuidePopup) {
      try {
        ...
        if (videoRef.current) {
          // Estado do vídeo mantido após fechar formulário
        }
        
        // O PiP já está com o estado correto - não modificar
        if (pipVideoRef.current) {
          // Estado do PiP mantido após fechar formulário
        }
  ...
  useEffect(() => {
    ...
    if (showHumanChat || showChatbotPopup) {
      try {
        ...
        // Chat aberto - estado de som configurado
  ...
  useEffect(() => {
    const handleBeforeUnload = async () => {
      ...
      if (conversationId && hasActiveSession) {
        try {
          ...
          await closeGuideConversation('inovpartner-23bc8', conversationId, 'user', 'Fechada ao fechar separador');
        } catch (error) {
    ...
    const handlePageHide = async () => {
      ...
      if (conversationId && hasActiveSession) {
        try {
          // Tentar encerrar a conversa do guia diretamente primeiro
          await closeGuideConversation('inovpartner-23bc8', conversationId, 'user', 'Fechada ao sair da página');
        } catch (error) {
          console.error('Erro ao encerrar conversa diretamente:', error);
    ...
    useEffect(() => {
      ...
      const handleVideoPlay = () => {
        setVideoPlaying(true);
        // iOS: Vídeo play detectado
      };
      
      const handleVideoPause = () => {
        setVideoPlaying(false);
        // iOS: Vídeo pause detectado
      };
  ...
  useEffect(() => {
    if (pipVideoRef.current) {
      ...
      // PiP sincronizado com vídeo principal
    }
  ...
  async function callOpenRouterAI(userMessage: string) {
    try {
      ...
      const makeApiCall = async (timeoutMs: number, useFastModel: boolean = false) => {
          ...
          const response = await fetch('/api/ask', {
            ...
            body: JSON.stringify({
              ...
              opts: {
                ...
                temperature: 0.3,
                verbosity: "low",
                // Usar modelo mais rápido se especificado
                ...(useFastModel && { model: "gpt-4o-mini" }),
      ...
      // Tentativa 1: Timeout de 25 segundos com LLaMA (modelo principal)
      let response;
      try {
        response = await makeApiCall(25000);
      } catch (error) {
        // Tentativa 2: Timeout de 10 segundos com DeepSeek
        try {
          response = await makeApiCall(10000, true);
        } catch (error2) {
          // A pessoa fica à espera da resposta - não há fallback
          throw new Error('Timeout em ambos os modelos');
        }
      }
      ...
      if (data.text) {
        ...
        if (shouldShowHumanCTA && (guideVideos?.humanChatEnabled === true)) {
          const ctaButtonHtml = `\n\n<div style="margin-top:16px;display:flex;justify-content:flex-start">\n  <button class="vg-open-human-chat-btn" style="padding:12px 20px;background:linear-gradient(135deg,rgba(155, 42, 136, 1) 0%, rgba(158, 58, 176, 1) 51%, rgba(107, 14, 96, 1) 100%);color:#fff;border:none;border-radius:20px;font-weight:600;cursor:pointer;box-shadow:0 4px 15px rgba(81, 174, 205, 0.4);transition:all 0.3s ease;" onclick="(function(e){e.preventDefault();window.openGuiaReal();})()">FALAR COM GUIA REAL</button>\n</div>`;
        ...
        // Se parecer inglês ou resposta muito curta, aguardar resposta
        if (englishWordCount > 2 || responseText.length < 10) {
          throw new Error('Resposta em inglês ou muito curta');
        }
      ...
      // Se não conseguir extrair a resposta, aguardar
      throw new Error('Não foi possível extrair resposta');
    } catch (error) {
  ...
  async function handleGuideClick(e: React.MouseEvent) {
    ...
    if (videoRef.current) {
      try {
        ...
        // Vídeo principal pausado antes de abrir formulário
      } catch (error) {
    ...
    if (returnedFromAiAfterHuman) {
      ...
      if (conversationId && conversationId !== 'CLOSED') {
        try {
          await closeGuideConversation('inovpartner-23bc8', conversationId, 'system', 'Reabertura após chat IA');
        } catch {}
        deleteCookie('chat_conversation_id');
        setHasActiveSession(false);
        setCurrentConversation(null);
    ...
    if (conversationId && conversationId !== 'CLOSED' && userName && userContact) {
      ...
      if (videoRef.current) {
        ...
        unsubscribeRef.current = listenToGuideConversation('inovpartner-23bc8', conversationId, (conv) => {
          ...
          setHumanChatMessages((conv as any).messages as unknown as ChatMessage[]);
                      ...
                      // Verificar se a conversa foi encerrada pelo backoffice
            if ((conv as any).status === 'closed') {
              // Conversa encerrada pelo backoffice - fechando chat e controlando vídeo
              ...
              if (videoRef.current) {
                if (isDesktop) {
                  ...
                  videoRef.current.pause();
                  setVideoPlaying(false);
                  // PC: Vídeo pausado após conversa encerrada pelo backoffice
                } else {
                  ...
                  videoRef.current.play();
                  setVideoPlaying(true);
                  // Smartphone: Vídeo continuando após conversa encerrada pelo backoffice
                }
              }
      ...
      if (guideChatEntryCount >= 4) {
        // Limite de 4 entradas no chat atingido - forçando preenchimento de dados
      ...
      // Primeira entrada ou dados limpos - entrada número: newCount de 4
      ...
      if (videoRef.current) {
        try {
          ...
          // Vídeo principal pausado na primeira abertura
      ...
      const attemptPlay = async () => {
        try {
          ...
          try {
            ...
            // iOS: falha ao recuperar reprodução
            setVideoPlaying(false);
          }
        }
      };
  ...
  async function handleDownload() {
    ...
    try {
      ...
      // Para vídeos locais, fazer download direto sem API proxy
      if (url.startsWith('/') || url.startsWith(window.location.origin)) {
        ...
        // Vídeo externo - usar API proxy (se necessário)
  ...
  function handleChatbotSend(e: React.FormEvent) {
    ...
    // Mostrar indicador de digitação com mensagem personalizada
    setChatbotMessages(prev => [...prev, { from: 'bot', text: 'Guia virtual está a preparar a sua resposta...', metadata: { fromChatbot: true, isTyping: true } }]);
    ...
    callOpenRouterAI(chatbotInput)
      .then(response => {
        ...
        setChatbotMessages(prev => {
          ...
          if (newMessages.length > 0 && newMessages[newMessages.length - 1].metadata?.isTyping) {
  ...
  async function handleGuideFormSubmit(e: FormEvent) {
    ...
    try {
      ...
      const hasBotResponse = chatbotMessages.some(msg => msg.from === 'bot' && msg.text.trim().length > 0 && !msg.metadata?.isTyping);
      ...
      // Criar conversa no Firebase usando guideServices
      ...
      const conversationId = await createGuideConversation('inovpartner-23bc8', conversationData);
      ...
      // Contador de entradas resetado para 1 após preenchimento de dados
      ...
      setFormContact('');
        ...
        setTimeout(() => {
          ...
          if (!isDesktop) {
            ...
            // Restaurando estado de som após formulário
            ...
            try {
              ...
              if (videoRef.current) {
                videoRef.current.muted = targetMuted;
                videoRef.current.volume = targetMuted ? 0 : 1;
                // Vídeo principal - estado de som configurado
        ...
        const verifyConversationExists = async () => {
          try {
            const existingConversation = await getGuideConversation('inovpartner-23bc8', conversationId);
  ...
  async function handleGuideFormSubmitWithCachedData(formData: { name: string; contact: string }) {
    try {
      ...
      const cachedHasBotResponse = chatbotMessages.some(msg => msg.from === 'bot' && msg.text.trim().length > 0 && !msg.metadata?.isTyping);
      ...
      // ID da conversa criada e dados da conversa
      ...
      // Nova conversa criada automaticamente com dados em cache
      ...
      const verifyConversationExists = async () => {
        try {
          ...
          // Erro ao verificar conversa, tentando novamente
          // Tentar novamente após 2 segundos
          setTimeout(verifyConversationExists, 2000);
        }
      };
      ...
      // Função para configurar o listener após verificação
      const setupConversationListener = () => {
        // Configurando listener para conversa
        
        // IMPORTANTE: Verificar se ainda estamos a criar a conversa (evitar conflitos)
        if (!isCreatingNewConversation) {
          // Conversa não está mais a ser criada, cancelando configuração do listener
          return;
        }
        
        // Limpar qualquer listener existente antes de configurar um novo
        if (unsubscribeRef.current) {
          // Limpando listener existente antes de configurar novo
          unsubscribeRef.current();
          unsubscribeRef.current = null;
        }
        ...
        unsubscribeRef.current = listenToGuideConversation('inovpartner-23bc8', conversationId, (conversation) => {
        // Listener recebeu atualização da conversa
  ...
  async function handleConfirmClose() {
    try {
      ...
      if (conversationId && conversationId !== 'CLOSED') {
        await closeGuideConversation('inovpartner-23bc8', conversationId, 'user', 'Fechada pelo utilizador');
      }
  ...
  async function handleHumanChatSend(e: React.FormEvent) {
    ...
    try {
      ...
      // Enviar para o Firebase usando guideServices
      await sendGuideMessage('inovpartner-23bc8', currentConversation.id, {
        from: 'user',
  ...
  useEffect(() => {
    if (!showGuidePopup && !showChatbotPopup && !showHumanChat) {
      ...
      if (onShowActionButtonsChange) {
        onShowActionButtonsChange(!showStartButton);
    }
    }
  }, [showGuidePopup, showChatbotPopup, showHumanChat, showStartButton, onShowActionButtonsChange]);

  // Ref para o container do SVG
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Calcular o progresso do scroll (0 a 1)
      const progress = Math.max(0, Math.min(1, 1 - (rect.top / windowHeight)));
      
      // Aplicar scale diferente para mobile e desktop
      const isMobile = window.innerWidth <= 768;
      const baseScale = isMobile ? 0.5 : 0.8;
      const maxScale = isMobile ? 1.3 : 1.0;
  ...
  return (
      ...
      <div className={`${styles.bgVideoContainer} ${showChatbotPopup ? styles.chatbotOpen : ''} ${showHumanChat ? styles.humanChatOpen : ''} ${showActionButtons ? styles.started : ''}`}>
        ...
        {!showStartButton && (
          <MobileOptimizedVideo
            ref={videoRef}
            className={getVideoClassName(styles.bgVideo)}
            src={toStreamUrl(guideVideos?.welcomeVideoURL) || "/videos da pagina/videoprincipal.mp4"}
          ...
          >
            {(() => {
              ...
              const desktopSrc = captions?.desktop || `/guides/${guideSlug}/captions_desktop.vtt` || '/videos da pagina/Legendas pc.vtt';
              const tabletSrc = captions?.tablet || `/guides/${guideSlug}/captions_tablet.vtt` || '/videos da pagina/Legendas Tablet.vtt';
              const mobileSrc = captions?.mobile || `/guides/${guideSlug}/captions_mobile.vtt` || '/videos da pagina/Legendas Mobile.vtt';
        ...
        {showStartButton && (
          <div className={styles.welcomeOverlay}>
            ...
            <MobileOptimizedVideo
              ...
              className={getVideoClassName(styles.welcomeBackgroundVideo)}
            ...
            <div className={`${styles.chatbotPopup} ${showChatbotPopup ? styles.fullscreenPopup : ''}`}>
              <div className={styles.chatbotHeader}>
                <div className={styles.chatbotHeaderTitle}>
                  ...
                  <p className={styles.chatbotHeaderSubtitle}>Inov Partner</p>
                </div>
                <button 
              ...
              <div className={`${styles.chatbotContent} ${isAndroid ? styles['android-adjusted'] : ''}`}>
                {showChatbotWelcome && (
                  <div className={`${styles.chatbotWelcome} ${isAndroid && androidWelcomeHidden ? styles['android-hidden'] : ''}`}>
                    <h3>BEM-VINDO AO GUIA VIRTUAL</h3>
                    <p className={styles.chatbotSubtitle}>Inov Partner</p>
                    {showInstructions && (
                    ...
                    {chatbotMessages.map((msg, index) => (
                      ...
                      >
                        {msg.from === 'bot' ? (
                          <>
                            <Image 
                              src={guideVideos?.chatIconURL || "/iconchat.jpg"} 
                              alt="Chat AI" 
                              width={40}
                              height={40}
                            ...
                            >
                              {msg.metadata?.isTyping ? (
                                <div className={styles.typingMessage}>
                                  <span>Guia virtual está a preparar a sua resposta</span>
                                  <div className={styles.typingDots}>
                                    <span></span>
                                  </div>
                                </div>
                              ) : (
                                <div dangerouslySetInnerHTML={useSanitizedHtml(msg.text)} />
                              )}
                            </div>
                          </>
                        ) : (
              ...
              <div className={styles.fixedBottomContainer}>
                ...
                {(!showInstructions || (Array.isArray(chatbotMessages) && chatbotMessages.length > 0)) && (
                  <div className={styles.quickSuggestionsBar}>
                    ...
                    >
                      {suggestionsBlocked ? 'Aguarde...' : (guideVideos?.chatConfig?.button3Text || 'Pergunta 3')}
                    </button>
                    <div className={styles.disclaimerText}>
                      O Chat pode cometer erros. Considere verificar informações importantes com a nossa equipa.
                    </div>
                  </div>
                )}
    ...
    if (/android/i.test(navigator.userAgent)) {
      ...
      {/* Espaçador para ficar abaixo do vídeo fixo */}
      {!showActionButtons && (<div className={styles.afterVideoSpacer} />)}

      {/* Slider de Logos dos Clientes */}
      {showActionButtons && (
            ...
            <div className={styles.logosSlider}>
                <div className={styles.logosTrack}>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-1.png" alt="Cliente 1" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-2.png" alt="Cliente 2" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-3.png" alt="Cliente 3" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-4.png" alt="Cliente 4" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-5.png" alt="Cliente 5" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-6.png" alt="Cliente 6" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-7.png" alt="Cliente 7" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-8.png" alt="Cliente 8" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-9.png" alt="Cliente 9" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-10.png" alt="Cliente 10" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-11.png" alt="Cliente 11" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-12.png" alt="Cliente 12" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-13.png" alt="Cliente 13" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-14.png" alt="Cliente 14" />
                  </div>
                  <div className={styles.logoItem}>
                    <img src="/logosclientes/brancos/logo-cliente-15.png" alt="Cliente 15" />
                  </div>
      ...
      {/* Serviços (fora e acima das FAQs) */}
      {showActionButtons && (
        <div className={styles.servicesSection}>
          <div className={styles.servicesContainer}>
            ...
            <div className={styles.servicesGrid}>
              ...
              {/* Linha inferior - Outros 4 serviços */}
              <div className={styles.servicesBottomRow}>
      ...
      {/* Seção Áreas de Negócio */}
      {showActionButtons && (
        <div className={styles.businessAreasSection}>
          <div className={styles.businessAreasContainer}>
            <div ref={svgContainerRef} className={styles.svgContainer}>
              <Lottie 
                ...
                loop={false}
                autoplay={false}
                speed={0.15} // Reduzindo para 15% da velocidade normal
              />
            </div>
          </div>
        </div>
      ...
      {false && showHumanChat && (
        <div className={styles.chatbotPopupOverlay}>
          <div className={styles.chatbotPopup}>
            ...
            <div className={`${styles.chatbotMessages} ${styles.humanChat}`}>
      ...
      {!isDesktop && (
        ...
        >
          <PiPOptimizedVideo
            ref={pipVideoRef}
            className={getVideoClassName(styles.pipVideo)}
            src={toStreamUrl(guideVideos?.welcomeVideoURL) || "/videos da pagina/videoprincipal.mp4"}
            loop
            ...
            onError={(e: any) => {
              ...
              if (video.src !== "/videos da pagina/videoprincipal.mp4") {
                console.log('🔄 Usando vídeo de fallback para PiP');
                video.src = "/videos da pagina/videoprincipal.mp4";
              }
            }}
          />
      ...
      {/* Espaço para não ficar escondido pelo container fixo do chat no fundo
          ...
          à última secção. */}
      {/* Comentado: div com altura fixa que causava problemas de layout
      ...
      {!showChatbotPopup && !showHumanChat && !showGuidePopup && !showStartButton && (
        <footer style={{
          ...
          <div style={{ 
            ...
            © 2025 Inov Partner Todos os direitos reservados.
          </div>
        </footer>
      )}
